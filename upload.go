...
func (b *Bridge) customUploadToMax(ctx context.Context, uploadType maxschemes.UploadType, reader io.Reader, fileName string, contentLength int64) (*maxschemes.UploadedInfo, error) {
        ...
        // 2. Потоковая загрузка файла на CDN (multipart) через io.Pipe без буферизации в памяти.
        pr, pw := io.Pipe()
        ...
        cdnReq, err := http.NewRequestWithContext(uploadCtx, http.MethodPost, endpoint.Url, pr)
        if err != nil {
