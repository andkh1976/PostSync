...
func (r *sqliteRepo) GetCrosspostMaxChat(tgChatID int64, ownerID int64) (int64, string, bool) {
        ...
        var dir string
        err := r.db.QueryRow(
                "SELECT max_chat_id, direction FROM crossposts WHERE tg_chat_id = ? AND (? = 0 OR tg_owner_id = ? OR owner_id = ?) AND deleted_at = 0",
...
func (r *sqliteRepo) GetCrosspostTgChat(maxChatID int64, ownerID int64) (int64, string, bool) {
        ...
        var dir string
        err := r.db.QueryRow(
                "SELECT tg_chat_id, direction FROM crossposts WHERE max_chat_id = ? AND (? = 0 OR owner_id = ? OR tg_owner_id = ?) AND deleted_at = 0",
