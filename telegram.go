...
// webAppInfo используется для WebApp-кнопки (не поддерживается в tgbotapi v5.5.1 напрямую).
	...
	for {
		...
		case update, ok := <-updates:
			...
			if text == "/start" || text == "/help" {
				...
				if b.cfg.MiniAppURL != "" {
					helpMsg.ReplyMarkup = webAppKeyboard{
						InlineKeyboard: [][]webAppButton{
							{
								{
									Text:   "⚙️ Открыть панель управления",
									WebApp: webAppInfo{URL: b.cfg.MiniAppURL},
								},
							},
						},
					}
...
func (b *Bridge) forwardTgToMax(ctx context.Context, msg *tgbotapi.Message, maxChatID int64, caption string) {
	...
	// caption уже содержит entity-форматирование (markdown) и метку источника,
...
func (b *Bridge) handleTgChannelPost(ctx context.Context, msg *tgbotapi.Message) {
	...
	maxChatID, direction, ok := b.repo.GetCrosspostMaxChat(msg.Chat.ID, 0)
	if !ok {
		return
	}
	if direction == "max>tg" {
...
func (b *Bridge) handleTgCallback(_ context.Context, query *tgbotapi.CallbackQuery) {
	...
	if strings.HasPrefix(data, "cprb:") {
		...
		_, direction, ok := b.repo.GetCrosspostTgChat(maxChatID, 0)
		if !ok {
			return
		}
		title := parseTgCrosspostTitle(query.Message.Text)
...
func (b *Bridge) handleTgEditedChannelPost(ctx context.Context, edited *tgbotapi.Message) {
	...
	maxChatID, direction, linked := b.repo.GetCrosspostMaxChat(edited.Chat.ID, 0)
	if !linked {
		return
	}
	if direction == "max>tg" {
...
func (b *Bridge) CheckTgAdmin(chatID, userID int64) (bool, error) {
	...
	if chatInfo.Type == "private" {
		return true, nil // в личке юзер сам себе админ (или можно вернуть false)
	}
