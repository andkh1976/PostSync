...
func (b *Bridge) handleAPITaskCancel(w http.ResponseWriter, r *http.Request) {
	...
	task, err := b.repo.GetSyncTask(req.TaskID)
	if err != nil {
...
func (b *Bridge) handleAPIChannelsPair(w http.ResponseWriter, r *http.Request) {
	...
	ctx := r.Context()
	isAdminTg, err := b.CheckTgAdmin(req.TgChatID, owner.UserID)
	if err != nil || !isAdminTg {
		slog.Warn("API /channels/pair failed TgAdmin check", "owner", owner.UserID, "tgChat", req.TgChatID, "err", err)
		writeError(w, http.StatusForbidden, "Вы не являетесь администратором в канале TG или бот не добавлен")
		return
	}
